describe ParseLine do
  ...
  describe '.create_hash' do
    ...
    context "when we have a pipe delimiter" do
      ...
      let(:pipe_parsed_hash) { {"last_name"=>"Goyette", "first_name"=>"Timmothy", "middle_initial"=>"E", "campus"=>"London", "favorite_color"=>"Pacific Blue", "date_of_birth"=>"10-2-1964"} }
      it { is_expected.to eq pipe_parsed_hash }
    end
  end

  describe '.parsed_data' do
    subject { described_class.new(line).parsed_data }

    context "when we have a comma delimiter" do
      let(:line) { "Kirlin, Mckayla, Atlanta, 5/29/1986, Maroon\n" }
      let(:parsed_line) { ["Kirlin", "Mckayla", "Atlanta", "5/29/1986", "Maroon"] }
      it { is_expected.to eq parsed_line }
    end

    context "when we have a dollar delimiter" do
      let(:line) { "Nolan $ Rhiannon $ O $ LA $ 10-4-1974 $ Vivid Tangerine\n" }
      let(:parsed_line) { ["Nolan", "Rhiannon", "O", "LA", "10-4-1974", "Vivid Tangerine"] }
      it { is_expected.to eq parsed_line }
    end

    context "when we have a pipe delimiter" do
      let(:line) { "Goyette | Timmothy | E | London | Pacific Blue | 10-2-1964\n" }
      let(:parsed_line) { ["Goyette", "Timmothy", "E", "London", "Pacific Blue", "10-2-1964"] }
      it { is_expected.to eq parsed_line }
    end
  end
